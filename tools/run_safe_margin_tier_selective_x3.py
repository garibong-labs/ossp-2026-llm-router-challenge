...
"""Run the one frozen safe-margin tier-selective x3 experiment."""
...
EXPERIMENT_ID = "safe-margin-tier-selective-x3-v1"
REPORT_TYPE = "safe-margin-tier-selective-x3-report-v1"
BASE_COMMIT = "3fbdfe84f7a7ccee247d3cc0536d11ada6a21ec9"
PROTOCOL_PATH = ROOT / "experiments/safe-margin-tier-selective-x3/protocol.v1.json"
EXPECTED_PROTOCOL_SHA256 = "dd89fa67d75484ccb07483132d40d579c3bb0e7aab6bc2e8ba806309739ae689"
DEFAULT_REPORT = ROOT / "experiments/safe-margin-tier-selective-x3/report.v1.json"
...
class PremiumPlan:
    selected: Tuple[str, ...]
    predicted_ratio: float
    stages: Tuple[safe_margin.StageReport, ...]
    failed_closed: bool = False


FROZEN_TRAIN_EXPECTATION = {
    "baseline_score": Decimal("0.672982954545"),
    "candidate_score": Decimal("0.673295454545"),
    "weighted_delta": Decimal("0.0003125"),
    "positive_families": ("cruxeval", "gsm8k"),
    "fast_swaps": 9,
    "premium_changed": 29,
}
...
def verify_protocol(path: Path = PROTOCOL_PATH) -> Mapping[str, Any]:
    ...
    try:
        valid = (
            ...
            and value["candidate"]["scope"] == "Fast matched-spend x3; Balanced x1; Premium full efficiency-bucket x3"
    ...
    for relative, expected in value["production_sha256"].items():
        if file_sha256(ROOT / relative) != expected:
            raise ExperimentError(f"production file changed: {relative}")
    return value
...
def _x3_stage(
    *, step_from: str, step_to: str, selected: list[str],
    predictions: Sequence[safe_margin.EpisodePrediction], light_total: float,
    mean_light: float, current_total: float, budget: float, min_gain: float,
    max_step_ratio: float, max_step_load: float,
) -> Tuple[float, safe_margin.StageReport]:
    """Exact safe-margin allocation stage, changing only bucket density to x3."""
    ...
    for index, prediction in enumerate(predictions):
        ...
        try:
            ...
            step_ratio = prediction.costs[step_to] / prediction.costs[LIGHT]
            step_load = increment / mean_light
        except (KeyError, TypeError, ValueError, ZeroDivisionError) as exc:
            raise ExperimentError("malformed Premium prediction evidence") from exc
        if not all(math.isfinite(value) for value in (gain, increment, step_ratio, step_load)):
            raise ExperimentError("non-finite Premium prediction evidence")
    ...
    total = current_total
    promoted = groups_promoted = 0
    for key in sorted(groups, key=lambda item: (-item[0], item[1:])):
        members = groups[key]
        delta = math.fsum(
            predictions[i].costs[step_to] - predictions[i].costs[step_from]
            for i in members
        )
        if not math.isfinite(delta):
            raise ExperimentError("non-finite Premium group spend")
        ...
        for i in members:
            selected[i] = step_to
            promoted += 1
    return total, safe_margin.StageReport(
        step=f"{step_from}->{step_to}", considered=considered, eligible=eligible,
        promoted=promoted, groups_considered=len(groups),
        groups_promoted=groups_promoted, budget=budget / light_total,
...
def premium_x3_plan(
    ...
    """Exact full x3 policy from the frozen efficiency-bucket sibling."""
    if not predictions:
        raise ExperimentError("empty Premium prediction evidence")
    config = safe_margin.TIER_PLAN_CONFIGS["premium"]
    try:
        light_total = math.fsum(item.costs[LIGHT] for item in predictions)
    except (KeyError, TypeError, ValueError) as exc:
        raise ExperimentError("malformed Premium prediction evidence") from exc
    if not math.isfinite(light_total) or light_total <= 0:
        raise ExperimentError("invalid Premium light spend")
    mean_light = light_total / len(predictions)
    full_budget = light_total * min(
        config.target_ratio, float(policy.tiers["premium"].budget_multiplier)
    )
    selected = [LIGHT] * len(predictions)
    stages = []
    total, stage = _x3_stage(
        step_from=LIGHT, step_to=AX31, selected=selected,
        predictions=predictions, light_total=light_total, mean_light=mean_light,
        current_total=light_total, budget=full_budget,
        min_gain=config.ax31_min_gain, max_step_ratio=config.ax31_max_step_ratio,
        max_step_load=config.ax31_max_step_load,
    )
    stages.append(stage)
    think_allowance = (full_budget - light_total) * config.think_budget_share
    total, stage = _x3_stage(
        step_from=AX31, step_to=THINK, selected=selected,
        predictions=predictions, light_total=light_total, mean_light=mean_light,
        current_total=total, budget=min(full_budget, total + think_allowance),
        min_gain=config.think_min_gain, max_step_ratio=config.think_max_step_ratio,
        max_step_load=config.think_max_step_load,
    )
    stages.append(stage)
    ratio = total / light_total
    if not math.isfinite(ratio) or ratio > min(config.target_ratio, float(policy.tiers["premium"].budget_multiplier)) + 1e-12:
        raise ExperimentError("Premium x3 budget invariant failed")
    return PremiumPlan(tuple(selected), ratio, tuple(stages))
...
def candidate_submission(
...
) -> Tuple[Submission, CandidatePlan | PremiumPlan | None]:
    ...
    try:
        plan = (
            matched_fast_plan(prediction_values, policy)
            if tier == "fast"
            else premium_x3_plan(prediction_values, policy)
        )
    except (ExperimentError, ArithmeticError, KeyError, TypeError, ValueError):
        # Relevant-tier fail closure is x1; the gate records the discrepancy.
        selected = tuple(decision.model_id for decision in baseline.submission.decisions)
        if tier == "fast":
            plan = CandidatePlan(
                selected, baseline.predicted_budget_ratio,
                baseline.predicted_budget_ratio, selected.count(AX31), 0, 0.0, 0.0
            )
        else:
            plan = PremiumPlan(
                selected, baseline.predicted_budget_ratio, baseline.stages,
                failed_closed=True,
            )
...
def decide_train_gate(evidence: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    try:
        ...
        checks = {
            ...
            "positive_families": positive >= 2,
            "nonnegative_families": nonnegative == 9,
            "worst_family_delta": worst >= 0,
            "balanced_exact_x1": evidence["balanced_exact_x1"] is True,
            ...
            "premium_x3_guards_pass": evidence["premium_x3_guards_pass"] is True,
            "all_tier_budgets_pass": evidence["all_tier_budgets_pass"] is True,
            "deterministic_repeated_output": evidence["deterministic_repeated_output"] is True,
            "frozen_expectation_exact": evidence["frozen_expectation_exact"] is True,
        }
        return {
            "checks": checks,
...
def _plans(data: EvaluationData) -> Tuple[
    Tuple[Submission, ...], Tuple[Submission, ...], CandidatePlan, PremiumPlan
]:
    ...
    fast_plan = pairs[0][1]
    premium_plan = pairs[2][1]
    if not isinstance(fast_plan, CandidatePlan) or not isinstance(premium_plan, PremiumPlan):
        raise ExperimentError("missing tier-selective candidate plan")
    return baseline, tuple(pair[0] for pair in pairs), fast_plan, premium_plan
...
def _common_evidence(data: EvaluationData) -> Mapping[str, Any]:
    baseline, candidate, fast_plan, premium_plan = _plans(data)
    ...
    tier_ratios = {
        "fast": fast_plan.predicted_ratio,
        "balanced": safe_margin.make_safe_margin_submission(
            data.inputs, data.policy, safe_margin.load_artifact(safe_margin.DEFAULT_ARTIFACT_PATH), "balanced"
        ).predicted_budget_ratio,
        "premium": premium_plan.predicted_ratio,
    }
    budgets_pass = all(
        tier_ratios[tier] <= float(data.policy.tiers[tier].budget_multiplier) + 1e-12
        for tier in TIERS
    )
    premium_changed = sum(
        left != right for left, right in zip(_models(candidate[2]), _models(baseline[2]))
    )
    return {
        "baseline": baseline,
        "candidate": candidate,
        ...
        "balanced_exact_x1": _models(candidate[1]) == _models(baseline[1]),
        ...
        "premium_x3_guards_pass": not premium_plan.failed_closed and budgets_pass,
        "all_tier_budgets_pass": budgets_pass,
        "deterministic_repeated_output": repeated,
        "tier_predicted_ratios": tier_ratios,
        "premium_changed_decisions": premium_changed,
        "fast_plan": {
...
def evaluate_train(train_input: Path = DEFAULT_TRAIN_INPUT, train_outcomes: Path = DEFAULT_TRAIN_OUTCOMES) -> Mapping[str, Any]:
    ...
    positive_names = tuple(
        name for name in risk_validation.FAMILY_LABELS if _decimal(family_deltas[name]) > 0
    )
    expectation_exact = (
        _decimal(evidence["baseline_score"]["final_score"]) == FROZEN_TRAIN_EXPECTATION["baseline_score"]
        and _decimal(evidence["candidate_score"]) == FROZEN_TRAIN_EXPECTATION["candidate_score"]
        and _decimal(evidence["weighted_score_delta"]) == FROZEN_TRAIN_EXPECTATION["weighted_delta"]
        and positive_names == FROZEN_TRAIN_EXPECTATION["positive_families"]
        and all(_decimal(value) >= 0 for value in family_deltas.values())
        and evidence["fast_plan"]["matched_swaps"] == FROZEN_TRAIN_EXPECTATION["fast_swaps"]
        and evidence["premium_changed_decisions"] == FROZEN_TRAIN_EXPECTATION["premium_changed"]
    )
    gate = decide_train_gate({
        **evidence, "family_deltas": family_deltas,
        "frozen_expectation_exact": expectation_exact,
    })
    return {
        ...
        "load_count": 1,
        "evaluation_count": 1,
        "balanced_exact_x1": evidence["balanced_exact_x1"],
        ...
        "premium_x3_guards_pass": evidence["premium_x3_guards_pass"],
        "all_tier_budgets_pass": evidence["all_tier_budgets_pass"],
        "tier_predicted_ratios": evidence["tier_predicted_ratios"],
        "premium_changed_decisions": evidence["premium_changed_decisions"],
        "frozen_expectation_exact": expectation_exact,
...
def evaluate_dev_loaded(data: EvaluationData) -> Mapping[str, Any]:
    ...
    return {
        "accessed": True, "load_count": 1, "evaluation_count": 1,
        "baseline_score": evidence["baseline_score"],
        "candidate_score": evidence["candidate_score_detail"],
...
def _candidate_risk_runner(data: risk_validation.SplitData) -> risk_validation.PolicyRunner:
    ...
    def plan(indices: Sequence[int], tier: str) -> Tuple[str, ...]:
        ...
        if tier == "premium":
            try:
                return premium_x3_plan(subset, data.policy).selected
            except (ExperimentError, ArithmeticError, KeyError, TypeError, ValueError):
...
def run_experiment(*, train_input: Path = DEFAULT_TRAIN_INPUT, train_outcomes: Path = DEFAULT_TRAIN_OUTCOMES, dev_input: Path = DEFAULT_DEV_INPUT, dev_outcomes: Path = DEFAULT_DEV_OUTCOMES, report_path: Path = DEFAULT_REPORT) -> Mapping[str, Any]:
    ...
    report: Dict[str, Any] = {
        ...
        "candidate": {
            "fast": "group-atomic matched-spend x3",
            "balanced": "exact x1",
            "premium": "full efficiency-bucket x3",
        ...
        "production_sha256": protocol["production_sha256"],
        "decision": {"eligible": False, "reason": "Train gate not evaluated", "submission_default": "safe-margin"},
        "dev": {"accessed": False, "load_count": 0, "evaluation_count": 0, "gate_passed": False},
