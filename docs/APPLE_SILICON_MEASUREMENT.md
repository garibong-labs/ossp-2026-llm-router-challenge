...
## 참가자 로컬 측정과 공식 장비의 구분

참가자가 자신의 Mac에서 Colima `linux/arm64` VM을 띄워 같은 cgroup 한도로
무언가를 재는 것은 유용하지만, 그것은 **아키텍처 측정**이지 공식 장비 측정이
아닙니다. 세 가지를 섞지 마십시오.

| 측정 | 무엇을 말할 수 있나 | 무엇을 말할 수 없나 |
| --- | --- | --- |
| macOS 네이티브(`darwin/arm64`) | 코드가 Apple Silicon에서 동작한다 | `linux/arm64` 요구를 만족한다고 말할 수 없습니다 |
| 참가자 로컬 Colima `linux/arm64` | 공식 아키텍처에서 동결 한도 안에 들어간다 | 최종 자원 여유나 동점 레이턴시를 말할 수 없습니다 |
| 위 절차로 운영자가 수행한 공식 측정 | 한도 동결과 동점 레이턴시 | — |

로컬 Colima VM은 QEMU 기반이고 CPU·메모리·디스크 구성과 호스트 부하가 공식
장비와 다릅니다. 따라서 참가자 문서나 보고서에서 로컬 Colima 실행을 최종 대회
장비 결과로 표기해서는 안 됩니다. 이 저장소의 실험 근거 파일도 로컬 측정에는
`final_contest_hardware=false`를 남깁니다.
